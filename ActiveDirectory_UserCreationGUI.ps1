...
$SGSession = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri "URL_For_Onprem_Exchange"
...
if(($fNameTextbox.Text -eq "") -or ($lNameTextbox.Text -eq "") -or ($locationCombo.Text -eq "OU=") -or $mgrTextbox.Text -eq ""){
    ...
    if($VendorCheckbox.Checked){
        ...
        if ($Result -eq 1) {
            ...
            if ($VPNCheckbox.Checked -eq $true) {
                Add-ADGroupMember -Identity JTEKTNA_VPNUsers -Members $samNameTextbox.Text -Server $domainController
            }else{}
            ...
            if ($oracleTextbox.Text -ne "") {
                Send-MailMessage -From $sendFrom -To $oracleContact -Cc $Manager.mail -Bcc "OVERSEER" -SmtpServer smtpmail.nrb.inside `
            ...
            $RemoteRouting = $samNameTextbox.Text + "MAIL_ROUTING_ADDRESS"
